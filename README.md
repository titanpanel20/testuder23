...
| | |
|---|---|
| 🪄 **۶ پروتکل + ۵ انتقال** | VLESS / VMess / Trojan / Shadowsocks / Hysteria2 / WireGuard روی WS / XHTTP / gRPC / HTTPUpgrade / TCP |
| 📱 **پروفایل اپراتور** | همراه اول / ایرانسل / عمومی: `sockopt` (BBR، TFO، keepalive زیر NAT، اجبار IPv4) + XTLS Vision روی هر دو سمت سرور و لینک |
| 📦 **فایل کانفیگ آماده** | `singbox.json` / `clash.yaml` / `xray.json` برای همان لینک اشتراک — تنها راه رساندن fragment، xmux و sniff به اپ |
...
پنل موقع اولین بوت یک ادمین `TiTaN` **بدون رمز** می‌سازد تا دیپلوی تازه قابل‌دسترس باشد؛
این حالت بی‌نهایت باز نمی‌ماند:

1. تا `TITAN_DEFAULT_LOGIN_HOURS` ساعت (پیش‌فرض ۲۴) می‌توان با `TiTaN` و **رمز خالی** وارد شد
   (رمز اشتباه قبول نمی‌شود) — یا مستقیم `/setup` را باز کرد و نام‌کاربری + رمز را یک‌جا ست کرد.
2. بعد از آن ورود بدون رمز بسته می‌شود و پنل تا ابد درِ باز ندارد. اگر رمزی نساخته باشی،
   راه بازگشت: `TITAN_ADMIN_PASSWORD` (حداقل ۸ کاراکتر) را در متغیرهای محیطی ست کن و سرویس را
   ری‌استارت/ری‌دیپلوی کن — پنل همان رمز را ست می‌کند و چیزی پاک نمی‌شود.
3. تا وقتی رمز نسازی، بنر زرد «پنل هنوز رمز عبور ندارد» بالای همهٔ صفحات پنل می‌ماند.
4. ⚠️ از **تنظیمات → امنیت** یک رمز اختصاصی تعیین کنید (در حالت پیش‌فرض، «رمز عبور فعلی» را خالی بگذارید)
5. ورود و ساخت کاربر در بخش **کاربران**
...
- صفحهٔ وب لینک اشتراک: مرورگر که `/sub/<key>` را باز کند صفحهٔ فارسیِ مصرف/انقضا/QR/کانفیگ‌ها را می‌بیند، اپ‌ها همان بیز۶۴ را می‌گیرند
...
| `TITAN_ADMIN_PASSWORD` | خالی | اگر ستش کنی، رمز ادمین همین موقع بوت ست می‌شود و ورود بدون رمز بسته می‌شود (راه بازگشت اگر رمز را گم کردی) |
| `TITAN_DEFAULT_LOGIN_HOURS` | `24` | چند ساعت ورودِ بدونِ رمز مجاز است؛ `0` = از همان اول بسته، عدد منفی = بدون مهلت |
...
| `/sub/<key>/page` | GET | صفحهٔ وب لینک (گروه یا کاربر)؛ `text/html`خواه مرورگر، وگرنه بیز۶۴ |
...
- صفحهٔ عمومی `/sub/<key>` فقط همان کانفیگ‌ها را نشان می‌دهد که در خود لینک هست: نه نام/uid بقیهٔ اعضای گروه، نه هیچ مسیر `/api/`، نه دارایی بیرونی؛ `noindex` و `no-store` هم روی پاسخ گذاشته می‌شود.
