...
const I18N = (() => {
  const dict = {
    fa: {
      ...
      first_run_now: 'پنل هنوز رمز عبور ندارد؛ تا این وضعیت، هر کسی با نام TiTaN و بدون رمز می‌تواند وارد شود.',
      first_run_hours: 'این فرصت تا {h} ساعت دیگر باز است و بعد از آن فقط با رمز (یا TITAN_ADMIN_PASSWORD) وارد می‌شوی.',
      first_run_closed: 'مهلت ورود بدون رمز تمام شد. اگر رمزی نگذاشته‌ای، TITAN_ADMIN_PASSWORD را ست کن و سرویس را ری‌استارت کن.',
      first_run_set: 'ست کردن رمز',
    ...
    en: {
      ...
      first_run_now: 'This panel has no password yet — while that is true, anyone can sign in as TiTaN with an empty password.',
      first_run_hours: 'The window stays open for another {h} hour(s), then only a password (or TITAN_ADMIN_PASSWORD) gets in.',
      first_run_closed: 'The no-password window has closed. If you never set a password, define TITAN_ADMIN_PASSWORD and restart the service.',
      first_run_set: 'Set a password',
